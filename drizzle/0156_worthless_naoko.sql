...
DROP VIEW IF EXISTS "agent_query"."customers";--> statement-breakpoint
ALTER TABLE "sales"."customer_contacts" DROP COLUMN IF EXISTS "notes";--> statement-breakpoint
ALTER TABLE "sales"."customers" DROP COLUMN IF EXISTS "notes";--> statement-breakpoint
CREATE VIEW "agent_query"."customers" AS
SELECT c.id, c.name, c.account_state, c.account_priority, c.email, c.phone, c.created_at
FROM "sales"."customers" c
WHERE c.deleted_at IS NULL AND c.organization_id = "agent_query".current_org();--> statement-breakpoint
GRANT SELECT ON "agent_query"."customers" TO erp_agent_ro;