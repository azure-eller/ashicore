...
CREATE TABLE IF NOT EXISTS "planning"."organization_planning_settings" (
	"organization_id" text PRIMARY KEY NOT NULL,
	"default_daily_manufacturing_capacity" numeric(12, 4),
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL
);
--> statement-breakpoint
ALTER TABLE "planning"."organization_planning_settings" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
...
ALTER TABLE "inventory"."items" ADD COLUMN IF NOT EXISTS "production_lead_time_days" numeric(8, 2);--> statement-breakpoint
ALTER TABLE "inventory"."items" ADD COLUMN IF NOT EXISTS "daily_capacity" numeric(12, 4);--> statement-breakpoint ALTER TABLE "purchasing"."supplier_items" DROP CONSTRAINT IF EXISTS "supplier_items_supplier_id_suppliers_id_fk";--> statement-breakpoint
...
CREATE UNIQUE INDEX IF NOT EXISTS "supplier_items_org_preferred_item_uidx" ON "purchasing"."supplier_items" USING btree ("organization_id","item_id") WHERE deleted_at IS NULL AND is_preferred = true;--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "organization_planning_settings_org_id_idx" ON "planning"."organization_planning_settings" USING btree ("organization_id");--> statement-breakpoint DROP POLICY IF EXISTS "supplier_items_org_isolation" ON "purchasing"."supplier_items";--> statement-breakpoint
CREATE POLICY "supplier_items_org_isolation" ON "purchasing"."supplier_items" AS PERMISSIVE FOR ALL TO public USING (organization_id = current_setting('app.current_org_id', true)) WITH CHECK (organization_id = current_setting('app.current_org_id', true));--> statement-breakpoint DROP POLICY IF EXISTS "organization_planning_settings_org_isolation" ON "planning"."organization_planning_settings";--> statement-breakpoint
CREATE POLICY "organization_planning_settings_org_isolation" ON "planning"."organization_planning_settings" AS PERMISSIVE FOR ALL TO public USING (organization_id = current_setting('app.current_org_id', true)) WITH CHECK (organization_id = current_setting('app.current_org_id', true));--> statement-breakpoint
ALTER TABLE "purchasing"."supplier_items" FORCE ROW LEVEL SECURITY;--> statement-breakpoint
ALTER TABLE "planning"."organization_planning_settings" FORCE ROW LEVEL SECURITY;--> statement-breakpoint
GRANT USAGE ON SCHEMA "planning" TO app_user;--> statement-breakpoint
GRANT USAGE ON SCHEMA "purchasing" TO app_user;--> statement-breakpoint
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE "purchasing"."supplier_items" TO app_user;--> statement-breakpoint
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE "planning"."organization_planning_settings" TO app_user;
