...
const fieldVariants = cva(
  "group/field flex w-full gap-2 data-[invalid=true]:text-[var(--status-danger-ink)]",
  {
    variants: {
      orientation: {
...
function FieldDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "text-left text-sm leading-normal font-normal text-[var(--color-ink-faint)] group-has-data-horizontal/field:text-balance [[data-variant=legend]+&]:-mt-1.5",
        "last:mt-0 nth-last-2:-mt-1",
        "[&>a]:underline [&>a]:underline-offset-4 [&>a:hover]:text-[var(--color-accent-ink)]",
        className
      )}
      {...props}
    />
...
function FieldSeparator({
  ...
  return (
    ...
    >
      ...
      {children && (
        <span
          className="relative mx-auto block w-fit bg-[var(--color-bg)] px-2 text-[var(--color-ink-faint)]"
...
function FieldError({
  ...
  return (
    <div
      ...
      className={cn("text-sm font-normal text-[var(--status-danger-ink)]", className)}
      {...props}
    >
      {content}
    </div>
