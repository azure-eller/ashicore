...
import { retryXeroEmailForPurchaseOrder } from "@/app/(dashboard)/purchasing/queries";
...
export const POST = apiHandler(async (request: Request, ctx: unknown) => {
  ...
  try {
    const result = await retryXeroEmailForPurchaseOrder(id);
    await tryRecordAccountingAuditEvent({
      organizationId: context.orgId,
      actor: { type: "user", userId: context.userId },
      eventType: "xero_email",
      outcome: "success",
      source: "POST /api/purchase-orders/[id]/xero-email",
