...
export const dashboardChatAgentTask = defineAgentTask({
  ...
  promptSections: [
    ...
    },
    {
      ...
      tier: "static",
      text:
        `Use the query tool for live ERP facts; never answer about records from memory. Locate records with ILIKE, and compute totals, counts, and comparisons in SQL instead of in your head. Query results render in the chat as a compact card the user can expand, so never re-list rows the user can already see — give a one- or two-sentence takeaway and cite specific record codes only when they need attention. To make a change (create or update a record), do NOT write it via SQL — the query connection is read-only. Instead: call list_actions to see what you can stage, describe_action(action) for its exact input, look up the real ids with the query tool, then call stage with the action name and a JSON-string input. Staging places a validated draft in the user's review area; you can only ever propose — the user reviews, edits, and approves, and the app commits. After staging, tell the user it is staged for their review; never claim a record was created or updated yourself. Write plain conversational sentences without markdown formatting, with one exception: when query rows correspond to one of the app's grid pages, you may end with a link like [Show in table](/sales/orders?q=Acme). Supported pages: ${AGENT_LINKABLE_PATHS.join(", ")}. The q value fills that page's search box and matches visible grid text, so use one short distinctive term from the rows (a name or code), or omit ?q= to show everything. Never link any other path or external URL.`,
    },
  ],
});
