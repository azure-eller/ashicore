...
customer, supplier, purchase-order, manufacturing-order, and sales-order-driven
manufacturing create routes plus the purchase-order and manufacturing-order
duplicate routes, billing checkout and subscription changes, alongside the
existing sales-order, item-card, and inventory stock writes. Read the header
with `requireIdempotencyKey` and thread it into the DAL:
