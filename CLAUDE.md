...
- **Neon preview cleanup.** After a PR is merged or closed, delete its Neon `preview/<git-branch>` branch before cleaning up the worktree; never delete `production`, `vercel-dev`, a protected branch, or a preview for an open PR.
