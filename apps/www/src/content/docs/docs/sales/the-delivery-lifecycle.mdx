...
description: How sales orders move from not shipped to partially shipped to shipped history.
sidebar:
...
A sales order stores only `open` or `done`. The UI derives delivery state from shipped and cancelled quantities.
...
| Shipped | Status is `done` or all ordered quantity has shipped or been cancelled. | The order is closed and read-only. |
...
| Step | Result |
| --- | --- |
| Consume stock | Delivered sales quantities convert with the line snapshot, then pull stocking quantities from lot-backed stock at the selected ship-from location. |
| Record shipped quantities | Each delivered line records matching sales-unit and stocking-unit shipped quantities. |
...
| Cancel a remainder | A partially shipped order can cancel undelivered quantities, release demand, and close without setting `shippedAt`. |
...
| Cancel remaining items | Partially shipped | Shipped | Keeps shipped history, cancels the undelivered balance, releases demand, and leaves `shippedAt` unchanged. |
