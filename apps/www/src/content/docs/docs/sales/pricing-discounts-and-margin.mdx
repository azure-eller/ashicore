...
1. The item's base selling price per effective sales unit becomes `listUnitPrice`.
...
Schedule-based pricing is included on Free and Pro. Quantity-break thresholds are in the sales unit: a break at 10 means 10 pallets on a Pallet line.
...
| Amount | Formula |
| --- | --- |
| Line subtotal | `sales quantity * price per sales unit` |
...
Before delivery, line margin can use estimated unit cost. Ashicore converts the current stock-unit cost to the line's snapshotted sales unit before comparing it with the sales price:

| Estimated field | Meaning |
| --- | --- |
| `estimatedUnitCost` | Current estimated cost for one sales unit. |
...
| --- | --- |
| `actualUnitCost` | Actual consumed stock cost per shipped sales unit. |
...
Changing an item's sales-unit conversion can rescale its default price to keep
the same underlying stock value. Existing order-line prices and unit snapshots
stay unchanged. See [Sales vs. stock units](/docs/sales/sales-vs-stock-units).
