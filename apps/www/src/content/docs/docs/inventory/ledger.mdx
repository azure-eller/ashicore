...
The inventory ledger is the audit trail. Stock movements, planning projections, disposition moves, and cost-only changes are appended as events; hot balance tables keep the current operating answer fast to read.
...
Text search matches item family names, SKUs, and assigned variant values. You
can paste a full displayed name such as `Premium Potting Mix / 2 ft3` to find
the concrete variant as well as search for either part on its own.

## Hot balances maintained beside it
...
| `inventory_item_balances` | item x location | On hand from current lot balances, demand and expected from active summaries, available to promise, last verified date. |
