...
| Sales unit | You | Optional family unit used on sales-order lines when a sellable item is ordered differently than it is stocked. |
| Unit conversion rate | You / system | Stocking units per sales unit, from 0.0001 through 99,999,999.9999 with no more than four meaningful decimal places; trailing zeroes are allowed. Ashicore derives compatible conversions; you enter incompatible conversions. |
...
| Default selling price | You | Prefill per effective sales unit. If there is no different sales unit, this is a price per stocking unit. |
| Sellable | You | Variant-level flag. Products and materials default to not sellable until enabled. Sellable items can appear on sales orders. |
...
| BOM locked | System / you | Prevents normal recipe edits. Authorized inventory administrators can edit or unlock a locked recipe; other permitted viewers see it read-only. |
