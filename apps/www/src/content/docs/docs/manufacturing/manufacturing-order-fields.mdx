...
| Quantity | decimal + product unit | product output quantity | Discrete planned output quantity in the product unit. Positive quantities must be between `0.0001` and `99,999,999.9999` with no more than four meaningful decimal places; trailing zeroes are allowed. |
| Number of batches | integer | 1 | Batch planned input when the product uses batch production; ingredient demand scales from this count. |
| Expected output | decimal + product unit | batch count × recipe yield | Batch planned output quantity; finished-good expected supply comes from this value. |
...
| Completed at | timestamp | blank | Set when the order is done and cleared if it returns to Work in progress. |

## Ingredients

| Field | Type | Default | Description |
| --- | --- | --- | --- |
| Ingredient | item reference | recipe component | Component item copied from the current product recipe, swapped while unpicked to a same-family variant configured on that recipe line, or added as a one-off line. |
| Default ingredient | item reference | recipe component | The original BOM component identity used to keep BOM-backed rows aligned when saving. |
...
| Planned quantity | decimal + unit | scaled recipe quantity | Planned consumption after scaling the recipe by output quantity or batches. Derived quantities round to four decimal places but cannot round to zero or exceed `99,999,999.9999`. |
...
| Planned output quantity | decimal + unit | order plan | Quantity expected from the batch or discrete completion. |
...
| Reversed quantity | decimal + unit | 0 | Quantity reversed from a recorded output row; compensating rows preserve the output history. |
...
| `done` | <Pill status="received" label="Done" /> | Completed production history; can return to Work in progress while its produced stock remains available. |
