...
| `stock-adjustment-route.spec.ts` | Stock adjustment route sets new on-hand and records a required reason in event metadata (rejects blank reason); for lot-tracked items, per-lot targets edit existing lots and create new lots from an operator-supplied lot number. Positive adjustments cost like stocktake gains: new lots and non-lot increases use the item's resolved unit cost (not zero), existing-lot increases preserve the lot's own cost, an unknown lotId 404s, and a replayed multi-lot key writes no duplicate events. |
