...
| `inventory-mutation-kernel.spec.ts` | Inventory mutations write events and reconcile lot/item projection truth, including stocktake count authority; billing plan state controls SKU-cap enforcement while counting every catalog item row, including archived rows. |
